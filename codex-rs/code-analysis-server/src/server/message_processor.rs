...
use crate::tool_config::create_code_analysis_tools;
use crate::code_analysis_bridge;
...
impl MessageProcessor {
    ...
    async fn handle_initialize(&mut self, id: RequestId, params: InitializeRequestParams) {
        ...
        let result = InitializeResult {
            ...
            instructions: Some("Code Analysis Server - Analyze code structure and find symbols".to_string()),
            protocol_version: params.protocol_version,
            server_info: Implementation {
                name: "code-analysis-server".to_string(),
    ...
    async fn handle_list_tools(&self, id: RequestId, _params: Option<ListToolsRequestParams>) {
        let tools = create_code_analysis_tools();
    ...
    async fn handle_call_tool(&self, id: RequestId, params: CallToolRequestParams) {
        ...
        let result = match params.name.as_str() {
            ...
            "find_symbol_references" => {
                match code_analysis_bridge::call_find_symbol_references(arguments) {
                    Ok(result) => CallToolResult {
                        content: vec![CallToolResultContent::TextContent(TextContent {
                            r#type: "text".to_string(),
                            text: serde_json::to_string_pretty(&result).unwrap_or_else(|_| result.to_string()),
                    ...
                    Err(e) => CallToolResult {
                        content: vec![CallToolResultContent::TextContent(TextContent {
                            r#type: "text".to_string(),
                            text: format!("Error finding symbol references: {}", e),
                            annotations: None,
                        })],
                        is_error: Some(true),
                    },
            ...
            "find_symbol_definitions" => {
                match code_analysis_bridge::call_find_symbol_definitions(arguments) {
                    Ok(result) => CallToolResult {
                        content: vec![CallToolResultContent::TextContent(TextContent {
                            r#type: "text".to_string(),
                            text: serde_json::to_string_pretty(&result).unwrap_or_else(|_| result.to_string()),
                    ...
                    Err(e) => CallToolResult {
                        content: vec![CallToolResultContent::TextContent(TextContent {
                            r#type: "text".to_string(),
                            text: format!("Error finding symbol definitions: {}", e),
                            annotations: None,
                        })],
                        is_error: Some(true),
                    },
            ...
            "get_symbol_subgraph" => {
                match code_analysis_bridge::call_get_symbol_subgraph(arguments) {
                    Ok(result) => CallToolResult {
                        content: vec![CallToolResultContent::TextContent(TextContent {
                            r#type: "text".to_string(),
                            text: serde_json::to_string_pretty(&result).unwrap_or_else(|_| result.to_string()),
                    ...
                    Err(e) => CallToolResult {
                        content: vec![CallToolResultContent::TextContent(TextContent {
                            r#type: "text".to_string(),
                            text: format!("Error getting symbol subgraph: {}", e),
                            annotations: None,
                        })],
                        is_error: Some(true),
                    },
