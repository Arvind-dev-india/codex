    ...
    match oauth_handler.get_token_status().await {
        Ok(status) => {
            if status.authenticated {
                println!("Status: Authenticated with Recovery Services");
                ...
                if !status.access_token_valid {
                    println!("Recommendation: Run 'recovery-services-server login' to refresh authentication");
                }
            } else {
                println!("Status: Not authenticated with Recovery Services");
                println!("Token location: {}/.codex/recovery_services_auth.json", codex_home.display());
