//! Authentication handling for Recovery Services (Azure Backup) API.
...
/// Microsoft's public client ID for Azure CLI (works for Recovery Services)
/// This is the same client ID used by Azure CLI and Azure PowerShell
const RECOVERY_SERVICES_CLIENT_ID: &str = "04b07795-8ddb-461a-bbee-02f9e1bf7b46";

/// Recovery Services (Azure Management) OAuth scopes
/// This scope provides access to Azure Management APIs including Recovery Services
const RECOVERY_SERVICES_SCOPES: &str = "https://management.azure.com/.default offline_access";
...
pub struct RecoveryServicesTokens {
...
/// Handles OAuth authentication for Recovery Services
pub struct RecoveryServicesOAuthHandler {
    codex_home: std::path::PathBuf,
}

impl RecoveryServicesOAuthHandler {
    ...
    pub async fn get_access_token(&self) -> Result<String> {
        tracing::info!("=== Recovery Services Authentication Flow ===");
    ...
    async fn device_code_flow(&self) -> Result<RecoveryServicesTokens> {
        let client = reqwest::Client::new();

        // Step 1: Get device code
        let device_request = DeviceCodeRequest {
            client_id: RECOVERY_SERVICES_CLIENT_ID,
            scope: RECOVERY_SERVICES_SCOPES,
        };
        ...
        eprintln!("\nRecovery Services (Azure Backup) Authentication Required");
        ...
        loop {
            ...
            if token_response.status().is_success() {
                ...
                eprintln!("Recovery Services authentication successful!");

                return Ok(RecoveryServicesTokens {
    ...
    async fn refresh_token(&self, refresh_token: &str) -> Result<RecoveryServicesTokens> {
        tracing::info!("Attempting to refresh Recovery Services access token...");
        let client = reqwest::Client::new();
    ...
    async fn load_tokens(&self) -> Result<RecoveryServicesTokens> {
        let auth_path = self.codex_home.join("recovery_services_auth.json");
        tracing::debug!("Looking for Recovery Services auth file at: {}", auth_path.display());
        
        if !auth_path.exists() {
            tracing::debug!("Recovery Services auth file does not exist");
            return Err(CodexErr::Other("No saved Recovery Services tokens found".to_string()));
        }
        
        let mut file = std::fs::File::open(&auth_path)
            .map_err(|e| {
                tracing::error!("Failed to open Recovery Services auth file: {}", e);
        ...
        file.read_to_string(&mut contents)
            .map_err(|e| {
                tracing::error!("Failed to read Recovery Services auth file: {}", e);
                CodexErr::Other(format!("Failed to read auth file: {}", e))
            })?;

        tracing::debug!("Recovery Services auth file contents length: {} bytes", contents.len());
        
        let tokens: RecoveryServicesTokens = serde_json::from_str(&contents)
            .map_err(|e| {
                tracing::error!("Failed to parse Recovery Services auth file: {}", e);
                tracing::debug!("Auth file contents: {}", contents);
                CodexErr::Other(format!("Failed to parse auth file: {}", e))
            })?;
            
        tracing::debug!("Successfully loaded Recovery Services tokens");
        Ok(tokens)
    }

    /// Save tokens to disk
    async fn save_tokens(&self, tokens: &RecoveryServicesTokens) -> Result<()> {
        let auth_path = self.codex_home.join("recovery_services_auth.json");
...
/// Authentication methods for Recovery Services
#[derive(Debug, Clone)]
pub enum RecoveryServicesAuth {
...
/// Recovery Services authentication handler
#[derive(Debug, Clone)]
pub struct RecoveryServicesAuthHandler {
    /// Authentication method
    pub auth: RecoveryServicesAuth,
}

impl RecoveryServicesAuthHandler {
    /// Create a new authentication handler using OAuth (device code flow)
    pub async fn from_oauth(codex_home: &Path) -> Result<Self> {
        // Use the dedicated Recovery Services OAuth handler with correct scopes
        let oauth_handler = RecoveryServicesOAuthHandler::new(codex_home);
        
        // Get access token with Recovery Services-specific scopes
        let access_token = oauth_handler.get_access_token().await?;
        
        Ok(Self {
            auth: RecoveryServicesAuth::OAuth(access_token),
        })
    }
    ...
    pub fn without_auth() -> Self {
        Self {
            auth: RecoveryServicesAuth::None,
        }
    }
    ...
    pub fn get_auth_header(&self) -> Option<String> {
        match &self.auth {
            RecoveryServicesAuth::OAuth(access_token) => {
                // OAuth uses Bearer token
                Some(format!("Bearer {}", access_token))
            }
            RecoveryServicesAuth::None => None,
        }
    }
}